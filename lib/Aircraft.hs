...
data AircraftState = AircraftState
  { aircraft :: Map.Map Word32 Aircraft
  -- ^ Map from ICAO to Aircraft
  , lastPrune :: POSIXTime
  -- ^ Last time old aircraft were pruned
  }
...
data Aircraft = Aircraft
  ...
  , category :: !(Maybe Mode.AircraftCategory)
  -- ^ Aircraft category
  , position :: !(Maybe Position)
  -- ^ Last known position
  , altitudeFt :: !(Maybe Int)
  -- ^ Last known altitude in feet
  , groundSpeed :: !(Maybe Int)
  ...
  -}
  , evenCPRLat :: !(Maybe Int)
  -- ^ Even frame latitude
  , evenCPRLon :: !(Maybe Int)
  -- ^ Even frame longitude
  , evenCPRTime :: !(Maybe Int)
  -- ^ Even frame reception time (ms)
  , oddCPRLat :: !(Maybe Int)
  -- ^ Odd frame latitude
  , oddCPRLon :: !(Maybe Int)
  -- ^ Odd frame longitude
  , oddCPRTime :: !(Maybe Int)
  -- ^ Odd frame reception time (ms)
  }
  deriving Show
...
updateAircraftFields ac msg now =
  ac
  ...
  ( newCallsign
    ...
    , updatedOddTime
      ...
      case Mode.msgSpecific msg of
        ...
        Just
          ( Mode.DF17Fields
              { Mode.esType = Mode.AircraftID
              , Mode.esData = Mode.ESAircraftID ident
              }
            ) ->
        ...
        Just
          ( Mode.DF17Fields
              { Mode.esType = Mode.AirbornePos
              , Mode.esData = Mode.ESAirbornePos pos alt
              }
            ) ->
        ...
        Just
          ( Mode.DF17Fields
              { Mode.esType = Mode.AirborneVel
              , Mode.esData = Mode.ESAirborneVel vel
              }
            ) ->
            case vel of
              ...
              Mode.AirVelocity{} ->
                -- We don't track air velocity for now
                ( callsign ac
                , category ac
                , position ac
                , altitudeFt ac
